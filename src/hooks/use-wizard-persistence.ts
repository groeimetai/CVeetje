...
export type WizardStep = 'linkedin' | 'job' | 'fit-analysis' | 'style-choice' | 'style' | 'template' | 'generating' | 'preview';
...
export function useWizardPersistence(): UseWizardPersistenceResult {
  ...
  const saveDraft = useCallback((draftData: Omit<WizardDraft, 'savedAt'>) => {
    // Don't save if we're in generating, preview, or template step (after CV is generated)
    if (draftData.currentStep === 'generating' || draftData.currentStep === 'preview' || draftData.currentStep === 'template') {
      return;
    }
