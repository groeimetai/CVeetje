...
import { DesignTweaksSheetV2 } from './design-tweaks-sheet-v2';
import {
  getVisibleFields,
  fieldsByTab,
  getAtPath,
...
/**
 * Wrapper export: routes on `tokens.engineVersion`. Legacy v1 CVs render the
 * full v1 sheet (everything below); v2 CVs render the new cv-engine sheet.
 * cv-preview.tsx imports this single name and doesn't need to know which
 * engine the doc was generated by.
 *
 * Belt-and-braces: also detects "v2-shaped" docs (recipeId present, but
 * engineVersion missing somehow) so a v2 token never reaches V1 — which
 * would crash on `colors.primary` access.
 */
export function DesignTweaksSheet(props: DesignTweaksSheetProps) {
  const t = props.tokens as unknown as { engineVersion?: string; recipeId?: string; colors?: unknown };
  const isV2 = t?.engineVersion === 'v2' || (typeof t?.recipeId === 'string' && !t?.colors);
  if (isV2) {
    return (
      <DesignTweaksSheetV2
        open={props.open}
        onOpenChange={props.onOpenChange}
        tokens={props.tokens as unknown as CVStyleTokensV2}
        originalTokens={(props.originalTokens as unknown as CVStyleTokensV2) ?? null}
        cvId={props.cvId}
        onTokensChange={tokens => props.onTokensChange(tokens as unknown as CVDesignTokens)}
      />
    );
  }
  return <DesignTweaksSheetV1 {...props} />;
}

export function DesignTweaksSheetV1({
  open,
  onOpenChange,
  tokens,
  originalTokens,
  ...
  // Defensive bail: v1 sheet renders controls that read tokens.colors.{primary,...}
  // via tweak-schema paths. If a v2-shaped token slips through (missing colors,
  // engineVersion not 'v2'), bail rather than crash. The wrapper should
  // already route, but this is belt-and-braces.
  if (!tokens || !(tokens as { colors?: unknown }).colors) {
    return null;
  }
