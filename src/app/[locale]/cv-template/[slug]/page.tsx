...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const p = getRolePage(locale as Locale, 'template', slug);
  if (!p) return { title: 'Niet gevonden' };
  return {
    title: p.title,
    description: p.description,
    keywords: p.keywords,
    alternates: { canonical: `/${locale}/cv-template/${slug}` },
    openGraph: {
      title: p.title,
      description: p.description,
      type: 'article',
      url: `${APP_URL}/${locale}/cv-template/${slug}`,
    },
  };
}

export default async function CvTemplatePage({ params }: Props) {
  const { locale, slug } = await params;
  ...
  return (
    <ArticleLayout
      ...
      title={p.title}
      description={p.hero}
      backHref="/cv-template"
      backLabel={nl ? 'Alle templates' : 'All templates'}
      ctaPrimary={{ href: '/register', label: nl ? 'Maak je eigen CV' : 'Build your CV' }}
      ctaSecondary={{ href: '/cv-template', label: nl ? 'Bekijk andere situaties' : 'See other situations' }}
    >
      <ArticleStructuredData
        headline={p.title}
        description={p.description}
        url={`/${locale}/cv-template/${slug}`}
      ...
      <BreadcrumbStructuredData
        items={[
          ...
          { name: p.title, url: `/${locale}/cv-template/${slug}` },
        ]}
      />
      ...
      <h2>{nl ? 'Concrete voorbeelden' : 'Concrete examples'}</h2>
      <ul>
